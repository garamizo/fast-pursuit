...
vec3 Centroid(const Triangle& t) {
    vec3 result;
    result.x = t.points[0].x + t.points[1].x + t.points[2].x;
    result.y = t.points[0].y + t.points[1].y + t.points[2].y;
    result.z = t.points[0].z + t.points[1].z + t.points[2].z;
...
vec3 Barycentric(const Point& p, const Triangle& t) {
    vec3 ap = p - t.points[0];
    vec3 bp = p - t.points[1];
    vec3 cp = p - t.points[2];

    vec3 ab = t.points[1] - t.points[0];
    vec3 ac = t.points[2] - t.points[0];
    vec3 bc = t.points[2] - t.points[1];
    vec3 cb = t.points[1] - t.points[2];
    vec3 ca = t.points[0] - t.points[2];
...
Plane FromTriangle(const Triangle& t) {
    Plane result;
    result.normal = Normalized(Cross(t.points[1] - t.points[0], t.points[2] - t.points[0]));
    result.distance = Dot(result.normal, t.points[0]);
    return result;
}
...
Point ClosestPoint(const Triangle& t, const Point& p) {
    ...
    Point c1 = ClosestPoint(Line(t.points[0], t.points[1]), closest); // Line AB
    Point c2 = ClosestPoint(Line(t.points[1], t.points[2]), closest); // Line BC
    Point c3 = ClosestPoint(Line(t.points[2], t.points[0]), closest); // Line CA
...
bool TriangleAABB(const Triangle& t, const AABB& a) {
    ...
    vec3 f0 = t.points[1] - t.points[0]; 
    vec3 f1 = t.points[2] - t.points[1]; 
    vec3 f2 = t.points[0] - t.points[2]; 
...
#else 
    vec3 t1_f0 = t1.points[1] - t1.points[0]; // Edge 0
    vec3 t1_f1 = t1.points[2] - t1.points[1]; // Edge 1
    vec3 t1_f2 = t1.points[0] - t1.points[2]; // Edge 2

    vec3 t2_f0 = t2.points[1] - t2.points[0]; // Edge 0
    vec3 t2_f1 = t2.points[2] - t2.points[1]; // Edge 1
    vec3 t2_f2 = t2.points[0] - t2.points[2]; // Edge 2
...
bool TriangleTriangleRobust(const Triangle& t1, const Triangle& t2) {
    vec3 axisToTest[] = {
        // Triangle 1, Normal
        SatCrossEdge(t1.points[0], t1.points[1], t1.points[1], t1.points[2]),
        // Triangle 2, Normal
        SatCrossEdge(t2.points[0], t2.points[1], t2.points[1], t2.points[2]),

        // Cross Product of edges
        SatCrossEdge(t2.points[0], t2.points[1], t1.points[0], t1.points[1]),
        SatCrossEdge(t2.points[0], t2.points[1], t1.points[1], t1.points[2]),
        SatCrossEdge(t2.points[0], t2.points[1], t1.points[2], t1.points[0]),

        SatCrossEdge(t2.points[1], t2.points[2], t1.points[0], t1.points[1]),
        SatCrossEdge(t2.points[1], t2.points[2], t1.points[1], t1.points[2]),
        SatCrossEdge(t2.points[1], t2.points[2], t1.points[2], t1.points[0]),

        SatCrossEdge(t2.points[2], t2.points[0], t1.points[0], t1.points[1]),
        SatCrossEdge(t2.points[2], t2.points[0], t1.points[1], t1.points[2]),
        SatCrossEdge(t2.points[2], t2.points[0], t1.points[2], t1.points[0]),
    };
...
bool TrianglePlane(const Triangle& t, const Plane& p) {
    float side1 = PlaneEquation(t.points[0], p);
    float side2 = PlaneEquation(t.points[1], p);
    float side3 = PlaneEquation(t.points[2], p);
...
bool ModelTriangle(const Model& model, const Triangle& triangle) {
    ...
    mat4 inv = Inverse(world);
    Triangle local;
    local.points[0] = MultiplyPoint(triangle.points[0], inv);
    local.points[1] = MultiplyPoint(triangle.points[1], inv);
    local.points[2] = MultiplyPoint(triangle.points[2], inv);
    if (model.GetMesh() != 0) {
...
void GetCorners(const Frustum& f, vec3* outCorners) {
    outCorners[0] = Intersection(f.planes[4], f.planes[0],    f.planes[2]);
    outCorners[1] = Intersection(f.planes[4], f.planes[0],    f.planes[3]);
    outCorners[2] = Intersection(f.planes[4], f.planes[1], f.planes[2]);
    outCorners[3] = Intersection(f.planes[4], f.planes[1], f.planes[3]);
    outCorners[4] = Intersection(f.planes[5],  f.planes[0],    f.planes[2]);
    outCorners[5] = Intersection(f.planes[5],  f.planes[0],    f.planes[3]);
    outCorners[6] = Intersection(f.planes[5],  f.planes[1], f.planes[2]);
    outCorners[7] = Intersection(f.planes[5],  f.planes[1], f.planes[3]);
}
